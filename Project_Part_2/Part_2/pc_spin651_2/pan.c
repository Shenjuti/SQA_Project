...
short src_ln0 [] = {
	  0,   9,  10,  12,  13,  14,  14,  15, 
	 11,  17,   8,  18,   8,  18,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "Reservation.pml", 1, 13 },
	{ "-", 14, 15 }
};
uchar reached0 [] = {
	  0,   1,   0,   1,   0,   1,   0,   0, 
...
{	int j = 0, h = now._nr_pr;
	...
	case 2:	/* Reserve2 */
		...
		((P2 *)pptr(h))->elsebit = 0;
#ifdef VAR_RANGES
		logval("Reserve2:elsebit", ((P2 *)pptr(h))->elsebit);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Reserve1 */
		...
		((P1 *)pptr(h))->recvbit = 0;
#ifdef VAR_RANGES
		logval("Reserve1:recvbit", ((P1 *)pptr(h))->recvbit);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Customer */
		...
		((P0 *)pptr(h))->elsebit = 0;
#ifdef VAR_RANGES
		logval("Customer:sendbit", ((P0 *)pptr(h))->sendbit);
		logval("Customer:recvbit", ((P0 *)pptr(h))->recvbit);
		logval("Customer:elsebit", ((P0 *)pptr(h))->elsebit);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.toS = addqueue(calling_pid, 1, 0);
		now.toR = addqueue(calling_pid, 2, 0);
#ifdef VAR_RANGES
#endif
}
...
#endif
	switch (n) {
	case 2: j = sizeof(Q2); q_flds[2] = 3; q_max[2] = 3; break;
	case 1: j = sizeof(Q1); q_flds[1] = 3; q_max[1] = 3; break;
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.toS) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.toS) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.toS) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.toS) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.toR) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.toR) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.toR) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.toR) )))
			    ) return 0; break;
		...
		case 7: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P2 *)_this)->in) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P2 *)_this)->in) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P2 *)_this)->in) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P2 *)_this)->in) )))
			    ) return 0; break;
		case 8: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P2 *)_this)->out) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P2 *)_this)->out) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P2 *)_this)->out) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P2 *)_this)->out) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  RESERVATION:	3\n");
	printf("	mtype  BOOKING_SUCCESSFULLY:	2\n");
	printf("	mtype  ALREADY_BOOKED:	1\n");
	printf("	chan toS (=%d):	len %d:\t", now.toS, q_len(now.toS));
	c_chandump(now.toS);
	printf("	chan toR (=%d):	len %d:\t", now.toR, q_len(now.toR));
	c_chandump(now.toR);
}
void
...
{	/* int i; */
	...
	case 3:
		...
		break;
	case 2:
		printf("local vars proc %d (Reserve2):\n", pid);
	printf("	chan in (=%d):	len %d:\t", ((P2 *)pptr(pid))->in, q_len(((P2 *)pptr(pid))->in));
	c_chandump(((P2 *)pptr(pid))->in);
	printf("	chan out (=%d):	len %d:\t", ((P2 *)pptr(pid))->out, q_len(((P2 *)pptr(pid))->out));
	c_chandump(((P2 *)pptr(pid))->out);
	printf("	bit    elsebit:	%d\n", ((P2 *)pptr(pid))->elsebit);
		break;
	case 1:
		printf("local vars proc %d (Reserve1):\n", pid);
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("ALREADY_BOOKED"); return;
	case 2: Printf("BOOKING_SUCCESSFULLY"); return;
	case 3: Printf("RESERVATION"); return;
